/**
 * Template generators for workflow components
 */
import { AppOptions, ProjectConfig } from '../types';
...
export function generateAppOptionsTemplate(config: {
  ...
  const template: AppOptions = {
    id: config.id,
    code: config.code,
    title: config.title,
    description: config.description || '',
    status: config.status as 'active' | 'inactive' | 'draft',
 ...
 */
export function generateEmptyBpmnTemplate(processCode: string, title?: string): string {
 ...
 */
export function generateProjectConfigTemplate(
  projectCode: string,
  projectId: string
): string {
